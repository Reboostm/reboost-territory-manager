...
function generateActionItems({ gmb, ranking, website }) {
  ...
  if (!ranking?.mapPackPosition || ranking.mapPackPosition > 3) {
    items.push({
      ...
      description: "80% of local clicks go to the top 3 results. You need more reviews, a complete GMB profile, consistent citations, and local backlinks to compete.",
    });
  }
...
// ─── Parallel Places Lookup ───────────────────────────────────────────────────

async function lookupGmb(lead) {
  try {
    const place = await findBusiness(lead.businessName, lead.city, lead.state);
    if (!place) return { found: false, score: 0 };
...
async function lookupRanking(lead) {
  try {
    const results = await getMapPackResults(lead.category, lead.city, lead.state);
    if (!results?.length) return { mapPackPosition: null, competitors: [], score: 0, keyword: `${lead.category} in ${lead.city}, ${lead.state}` };
    ...
    return { mapPackPosition: null, competitors: [], score: 0, keyword: '' };
  }
}

async function lookupWebsite(lead) {
  if (!lead.website) return null;
  try {
    const ps = await runPageSpeed(lead.website);
    if (!ps) return null;
    return { ...ps, score: scoreWebsite(ps) };
  } catch (e) {
...
export async function runAudit(lead) {
  ...
  // Run all three sources in parallel — cuts total time from ~15s → ~5s
  const [gmb, ranking, website] = await Promise.all([
    lookupGmb(lead),
    lookupRanking(lead),
    lookupWebsite(lead),
  ]);
